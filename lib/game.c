...
int game_get_next_square(cgame g, uint i, uint j, direction dir, uint dist) {
    ...
    if (ii < 0 || ii >= (int)g->nb_rows)
        return -1;
    if (jj < 0 || jj >= (int)g->nb_cols)
        return -1;
...
bool game_is_over(cgame g) {
    ...
    for (uint i = 0; i < g->nb_rows; i++) { /* row i */
        uint nb_zeros = _count_in_row(g, 0, i);
        uint nb_ones = _count_in_row(g, 1, i);
        if (nb_zeros != (g->nb_cols / 2))
            return false;
        if (nb_ones != (g->nb_cols / 2))
            return false;
    }
    ...
    for (uint j = 0; j < g->nb_cols; j++) { /* col j */
        ...
        if (nb_zeros != (g->nb_rows / 2))
            return false;
        if (nb_ones != (g->nb_rows / 2))
            return false;
    }
